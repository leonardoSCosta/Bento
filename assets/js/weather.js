...
const feelElement = document.querySelector('.weatherFeel p');
...
function getWeather(latitude, longitude) {
    ...
    fetch(api)
        ...
        .then(function (data) {
            ...
            let feels = Math.floor(data.main.feels_like - KELVIN);

            weather.temperature.value = tempUnit == 'C' ? celsius : (celsius * 9) / 5 + 32;
            let description = data.weather[0].description;
            weather.iconId = data.weather[0].icon;
            weather.feelsLike = tempUnit == 'C' ? feels : (feels * 9) / 5 + 32;
...
function displayWeather() {
    ...
    feelElement.innerHTML = `Sensação ${weather.feelsLike.toFixed(0)}°<span class="darkfg">${tempUnit}</span>`;
}
